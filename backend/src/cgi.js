...
const getData = async () => {
  const path = process.env.PATH_INFO.replace('/cgi/ThirdParty/code-editor-hby/index.cgi', '')
  ...
  if (path.indexOf('/api') === 0) {
    ...
    return { api: '/api/read', query: { path: `/var/apps/code-editor-hby/target/server/dist${assets}`, cache: 1 } }
  }
}
